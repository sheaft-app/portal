...
<div class="delivery-card rounded-3xl px-4 py-2 bg-white">
    <div class="flex justify-between py-2 border-b border-gray-300">
        ...
        <span class="rounded-full px-4 py-1 text-gray-800 bg-green-200 delivery-status">À livrer</span>
    </div>
    <div class="flex justify-between py-2 border-b border-gray-300 w-full">
        ...
        <div>
            ...
            <p class="text-primary text-2xl font-medium">{deliveryBatch.productsToDeliverCount}</p>
        </div>
    ...
    <div class="flex justify-between py-2 items-center flex-wrap space-x-4">
        <div>
            ...
            <p class="font-medium text-lg">Fabien Mugnier</p>
        </div>
        <button on:click={() => routerInstance.goTo(DeliveryBatchesRoutes.Edit, { id: deliveryBatch.id })} class="btn btn-lg btn-accent text-lg font-semibold">Vérifier</button>
    </div>
</div>
