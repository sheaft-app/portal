...
<div class="delivery-card rounded-3xl px-4 py-2 bg-white cursor-pointer" class:selected={selected == deliveryBatch.id} on:click={() => selected = deliveryBatch.id}>
    <div class="py-2 border-b border-gray-300">
        ...
        <p>entre {from.hours}h{from.minutes == 0 ? "00" : from.minutes} et {to.hours}h{to.minutes == 0 ? "00" : to.minutes}</p>
    </div>
