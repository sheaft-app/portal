...
<script>
	...
	onMount(async () => {
		...
		await query({
			...
			success: async (res) => {
				deliveries = res.deliveries.filter((d) =>
					[
						DeliveryBatchStatus.InProgress.Value,
						DeliveryBatchStatus.Ready.Value,
						DeliveryBatchStatus.Waiting.Value,
					].includes(d.status)
				);

                deliveries.map(d => d.products.map(p => (p.delivered = p.quantity)));
			},
				...
				<div in:fly|local={{ x: 800, duration: 400 }}>
					...
					<div>
						{#each delivery.products as product, index}
							<DeliverySummaryProduct
								{product}
								{index}
								isExpandedByDefault
								numberOfProducts={delivery.products.length - 1}
							/>
						{/each}
					</div>
