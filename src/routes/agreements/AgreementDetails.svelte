          ...
          shadow md:rounded md:mb-2"
				>
					...
					<div class="mt-2">
						...
						{:else}
							<p>Cet accord a été annulé, vous ne pouvez plus interagir avec.</p>
						{/if}
					</div>
				</div>
			{/if}
          ...
          md:rounded md:mb-2"
				>
					<p class="uppercase font-bold leading-none text-center">
						{isSender(agreement)
							? `Cet accord est en attente de validation du ${
									authManager.isInRole(Roles.Producer.Value) ? "magasin" : "producteur"
							  }.`
							: "Cet accord est en attente d'une réponse de votre part"}
					</p>
			...
			{#if authManager.isInRole(Roles.Store.Value)}
				<ProducerAgreement {agreement} {distanceInfos} />
			{:else}
				<StoreAgreement {agreement} {distanceInfos} />
			{/if}
		</div>
	</PageBody>
</TransitionWrapper>
