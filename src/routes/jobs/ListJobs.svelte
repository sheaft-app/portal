<script>
	...
	$: canResumeJobs =
		selectedItems.length > 0 &&
		selectedItems.filter((o) => o.status == ProcessStatusKind.Paused.Value).length == selectedItems.length;

	$: canPauseJobs =
		selectedItems.length > 0 &&
		selectedItems.filter((o) => o.status == ProcessStatusKind.Processing.Value).length == selectedItems.length;

	$: canRetryJobs =
		selectedItems.length > 0 &&
		selectedItems.filter((o) => o.status == ProcessStatusKind.Failed.Value).length == selectedItems.length;
	...
	<PageBody {errorsHandler}>
		...
		>
			...
			<td class="px-6 py-4 whitespace-no-wrap hidden md:table-cell">
				<span
					class="px-3 inline-flex text-xs leading-5 font-semibold rounded-full bg-{ProcessStatusKind.color(
						job.status
					)}-500
				text-white"
				>
