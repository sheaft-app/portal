<script>
  ...
  import { GET_JOB_DETAILS } from "./queries.js";
  import {
    faArchive,
    faCircleNotch,
    faTimesCircle,
  ...
  const getJob = async id => {
    isLoading = true;
    var res = await graphQLInstance.query(GET_JOB_DETAILS, { id }, errorsHandler.Uuid);
    isLoading = false;
  ...
  const handleJobsCommand = async (mutation, currentJob) => {
    isLoading = true;
    var res = await graphQLInstance.mutate(mutation, {
      ids: [currentJob.id]
    }, errorsHandler.Uuid);
    isLoading = false;
