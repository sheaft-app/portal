...
<TransitionWrapper style="margin: 0" hasRightPanel>
  <div class="details-container">
    ...
    {:else}
      <div>
      <div class:justify-between={productionSite} class="flex flex-wrap mt-5 mb-5">
          <img
            class="h-10 w-10 md:h-32 md:w-32 rounded-full p-1 md:mx-0 border
            border-gray-800 border-solid"
            src={producer.picture ? producer.picture : 'img/icons/farmer.svg'}
            alt="Producteur" />
          <div class="w-5/12 md:w-6/12 lg:pl-8 pl-3">
            <h3 class="font-bold uppercase">GAEC La Ferme du Parquet</h3>
            <p class="text-sm">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>
            <button class="btn btn-link">Lire plus</button>
          </div>
          {#if productionSite}
            <div class="lg:w-4/12 lg:block hidden">
              <div class="bg-white shadow-md rounded p-4 px-5 mt-2 w-full">
                <div class="flex flex-row justify-between items-start mb-3">
                  <div>
                    <p class="font-semibold">Site de production</p>
                    <p>{productionSite.address.line1}</p>
                    {#if productionSite.address.line2}
                      <p>
                        {productionSite.address.line2}
                      </p>
                    {/if}
                    <p>{productionSite.address.zipcode} {productionSite.address.city}</p>
                    <a 
                      class="mt-1"
                      target="_blank"
                      href={`https://www.google.com/maps/search/?api=1&query=${encodeQuerySearchUrl(productionSite.address)}`}>
