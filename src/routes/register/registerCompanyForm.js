...
export const normalizeCompany = (profile) => {
  ...
  return {
      ...
      legals: {
          ...
          vatIdentifier: (get(company).vatIdentifier && !get(company).notSubjectToVat) ? "FR" + get(company).vatIdentifier + get(siret).toString().substring(0, 9) : null,
          owner: {
              ...get(owner),
              address: {
                  ...get(owner).address,
...
export const normalizeStore = (profile) => omit({
