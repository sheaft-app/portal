<script>
	...
	export let catalog, invalidCatalogProducts = false, errorsHandler;

	const routerInstance = GetRouterInstance();
	const {open, clearApolloCache} = getContext('modal');

	const headers = [{label: 'Nom', mobile: true}, {label: 'Prix HT', mobile: true}, {
		label: 'Ajouté le',
		mobile: false
		...
		<tr class:bg-green-200={product.checked} class:bg-red-200={product.markForDeletion}>
			<td class="px-3 md:px-6 py-4 whitespace-no-wrap border-b border-gray-200">
				<div
					...
					{#if !product.markForDeletion}
						<button type="button" class="btn-link block lg:hidden" on:click={() => removeProduct(product)}>Retirer
						</button>
					{:else}
			...
			<td class="px-3 md:px-6 py-4 whitespace-no-wrap border-b border-gray-200 hidden lg:table-cell">
				<div
					...
					style="max-width: 180px;">
					{#if !product.markForDeletion}
						<button type="button" class="btn btn-outline btn-lg" on:click={() => removeProduct(product)}>Retirer
						</button>
					{:else}
						<button type="button" class="btn btn-outline btn-lg" on:click={() => cancelRemoveProduct(product)}>Annuler
						</button>
					{/if}
				</div>
			</td>
