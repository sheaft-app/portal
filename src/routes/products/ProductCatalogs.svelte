		...
		<tr class:bg-green-200={catalog.checked} class:bg-red-200={catalog.markForDeletion}>
			<td class="px-3 md:px-6 py-4 whitespace-no-wrap border-b border-gray-200">
				<div
					...
					{#if !catalog.markForDeletion}
						<button type="button" class="btn-link block lg:hidden" on:click={() => removeCatalog(catalog)}>Retirer
						</button>
					{:else}
			...
			<td class="px-3 md:px-6 py-4 whitespace-no-wrap border-b border-gray-200 hidden lg:table-cell">
				<div
					...
					style="max-width: 180px;">
					{#if !catalog.markForDeletion}
						<button type="button" class="btn btn-outline btn-lg" on:click={() => removeCatalog(catalog)}>Retirer
						</button>
					{:else}
						<button type="button" class="btn btn-outline btn-lg" on:click={() => cancelRemoveCatalog(catalog)}>Annuler
						</button>
					{/if}
				</div>
			</td>
