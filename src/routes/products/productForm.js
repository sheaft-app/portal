...
export const normalizeCreateProduct = product => omit({
        ...
        catalogs: product.catalogs ? product.catalogs.map((c) => ({
            id: c.id,
            wholeSalePricePerUnit: c.wholeSalePricePerUnit
        })) : [],
				pictures: product.pictures ? product.pictures.map(p => ({id: p.new ? null : p.id, data: p.new ? p.data : null, position: p.position})) : [],
...
export const normalizeUpdateProduct = product => ({
    ...
    catalogs: product.catalogs ? product.catalogs.filter((c) => !c.markForDeletion).map((c) => ({
        id: c.id,
        wholeSalePricePerUnit: c.wholeSalePricePerUnit
    })) : []
})
