...
>
	...
	<div class="form-control">
		<div class="w-full md:w-2/2">
			...
			<input bind:value={store.name} id="grid-line" type="text" placeholder="Nom commercial de votre magasin" />
		</div>
	</div>
	<input hidden required name="city" bind:value={store.address.city} />
	<input hidden required name="zipcode" bind:value={store.address.zipcode} />
	<input hidden required name="line1" bind:value={store.address.line1} />
	<input hidden required name="latitude" bind:value={store.address.latitude} />
	<input hidden required name="longitude" bind:value={store.address.longitude} />
	...
	<div class="form-control">
		<div class="w-full md:w-2/2">
			...
			<input bind:value={store.address.line2} id="grid-line2" type="text" placeholder="Bâtiment B n°113" />
		</div>
	</div>
	...
	<div class="form-control" style="display:block;">
		<div class="w-full">
			...
			<div class="text-xxs mb-3">
				Vous pouvez changer l'ordre des images en restant cliqué sur l'image et en la faisant glisser à la
				position voulue.
			</div>
			<GalleryConfigurator
				bind:elements={store.pictures}
				elementHeight="80"
				elementWidth="100"
				newElementMinHeight="200"
				newElementMinWidth="200"
			/>
		</div>
	</div>
	<div class="form-control">
		<div class="w-full md:w-2/2">
			...
			<textarea
				bind:value={store.summary}
				id="grid-line3"
				class:invalid={store.summary && store.summary.length > 300}
				placeholder="Une description courte qui s'affichera sur votre page de présentation."
			/>
			<p
				class="text-sm text-gray-600"
				class:font-semibold={store.summary && store.summary.length > 300}
				class:text-red-500={store.summary && store.summary.length > 300}
			>
				{store.summary ? store.summary.length : 0}
				/ 300 caractères max
			</p>
		</div>
	</div>
	<div class="form-control">
		<div class="w-full md:w-2/2">
			...
			<textarea
				...
				id="grid-line4"
				class:invalid={store.description && store.description.length > 1500}
				placeholder="Si vous souhaitez entrer plus dans le détail, n'hésitez pas à compléter cette zone."
			/>
			<p
				class="text-sm text-gray-600"
				class:font-semibold={store.description && store.description.length > 1500}
				class:text-red-500={store.description && store.description.length > 1500}
			>
	...
	<div class="form-control">
		<div class="w-full md:w-2/2 flex justify-between">
			<svg
				...
				/></svg
			>
			<input bind:value={store.website} id="grid-line7" type="url" placeholder="Adresse vers votre site web" />
		</div>
	</div>
	<h3 class="font-semibold uppercase mb-0 mt-5">Visibilité</h3>
	<span class="bg-primary h-1 w-20 mt-2 mb-6 block" />
	<div class="mt-5">
		<Toggle classs="ml-1" bind:isChecked={store.openForNewBusiness}>
			<div class="ml-2">
				<span class="ml-1"
					>Je veux que les producteurs puissent me contacter pour me proposer leurs produits</span
				>
			</div>
		</Toggle>
	</div>
