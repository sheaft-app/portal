<script>
	...
	const openAndLoad = async () => {
		...
		await query({
			...
			success: async (res) => {
				const deliveries =
					res.agreement && res.agreement.delivery ? [res.agreement.delivery] : await loadDeliveries(res.id);
					...
					<div class="mt-5 px-4">
						...
						<div class="flex flex-wrap">
							{#each producer.tags as tag}
								<span
									class="mx-2 mb-2 h-6 px-4 rounded-full text-xs font-semibold flex items-center bg-gray-100 text-gray-600"
									>{tag.name}</span
								>
							{/each}
						</div>
