<script>
	...
	const selectProducer = () => {
		selectedItem.set(producer);
	};

	export let producer,
		hoveredProducer = null,
		businessLocation = null,
		clicked = selectProducer;
      ...
      hidden"
		>
			{#if producer.agreement}
				<div class="absolute w-full h-full" style="z-index: 1; top: 40%;">
					...
					{/if}
					{#if producer.agreement.status == AgreementStatusKind.WaitingForStoreApproval.Value || producer.agreement.status == AgreementStatusKind.WaitingForProducerApproval.Value}
		...
		<div class="relative lg:block flex p-3 lg:p-0">
			...
			<div class="bg-white rounded-lg p-0 pl-3 lg:p-4 w-full">
              ...
              text-white px-2 py-1 rounded-full border"
