<script>
	...
	onMount(() => {
		distanceInfos =
			displayProducerData && product && product.producer && product.producer.address
				? GetDistanceInfos(
						values["latitude"],
						values["longitude"],
      ...
      hidden"
			>
				{#if $cart.products.find((i) => i.id == product.id)}
					<TransitionWrapper classNames="absolute w-full h-full" style="z-index: 1; top: 40%;">
        ...
        cover; background-position: top;"
          ...
          background-position: top;"
					...
					>
						{#if $cart.products.find((i) => i.id == product.id)}
							<div
								class="rounded-full p-1 w-6 h-6 text-center bg-white text-normal"
								style="
                line-height: 0;
                z-index: 1;
                box-shadow: rgba(0, 0, 0, .6) 0px 0px 5px 3px;"
						...
						{#if src.includes("pictures/tags/") && !$cart.products.find((i) => i.id == product.id)}
							<div class="absolute" style="bottom: 10%; font-size: .50rem;">
								<div class="text-white p-1 bg-gray-800">Aucune image</div>
							</div>
						{/if}
					</div>
					{#if displayProducerData && distanceInfos}
						<div class="text-center">
							<div
								class="w-full distance-badge inline-block text-xs font-bold px-2 py-1 rounded-full border"
            ...
            justify-between items-center hidden lg:block"
              ...
              tracking-wide hidden lg:block"
							...
							<div class="text-xs text-gray-600 uppercase font-semibold hidden lg:block">
								<p>
