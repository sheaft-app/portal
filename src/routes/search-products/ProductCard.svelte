<script>
	...
	export let product, hoveredProduct = null, displayProducerData = true, displayPriceData = true;
	...
	onMount(() => {
		distanceInfos = displayProducerData && product && product.producer && product.producer.address ? GetDistanceInfos(values["latitude"],
			values["longitude"],
	...
	$: productPrice = product ? (authInstance.isInRole(Roles.Store.Value) ? product.wholeSalePricePerUnit : product.onSalePricePerUnit) : null;
</script>
        ...
        cover; background-position: top;"
          ...
          background-position: top;"
                ...
                box-shadow: rgba(0, 0, 0, .6) 0px 0px 5px 3px;">
					...
					{#if displayProducerData && distanceInfos}
						<div class="text-center">
							<div class="w-full distance-badge inline-block text-xs font-bold px-2 py-1 rounded-full border"
				...
				<div class="bg-white rounded-lg p-0 pl-3 lg:p-4 w-full">
					...
					<div class="pr-8">
						{#if displayProducerData && distanceInfos}
							<div class="hidden lg:inline-block mb-2 text-xs font-bold px-2 py-1 rounded-full border"
									 style="color: {distanceInfos.color}; border-color: {distanceInfos.color};">
								<Icon
									data={faMapMarkerAlt}
									scale=".8"
            ...
            lg:text-sm">
              ...
              {product.rating || 'Aucun avis'}
						...
						{#if displayPriceData && productPrice && productPrice > 0}
							<div
					...
					<div class="flex mt-2 lg:mt-0 items-center justify-between">
						{#if displayPriceData && productPrice && productPrice > 0}
							<div
								...
								{formatMoney(productPrice)}{authInstance.isInRole(["STORE"]) ? 'HT' : ''}
								<div
									class="text-xxs">{formatConditioningDisplay(product.conditioning, product.quantityPerUnit, product.unit)}</div>
							</div>
						{/if}
						{#if displayPriceData && !authInstance.isInRole(["STORE", "PRODUCER"])}
							<div class="w-7/12 lg:w-full lg:mt-4">
								{#if !product.available || !productPrice || productPrice <= 0}
