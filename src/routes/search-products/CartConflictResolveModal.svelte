...
<div class="pb-2">
	<div class="flex justify-between bg-primary -mx-6 px-6 py-2 -mt-2 items-center md:rounded-t-l">
		<h3 class="text-lg font-semibold text-white mb-0">Choisir un panier</h3>
	</div>
	{#each $cart.conflicts as conflict, index}
		...
		{#each conflict.products as product}
			<div class="py-2 justify-between px-2 flex border-b border-gray-200 border-solid">
				<div class="w-1/12 h-12 text-right leading-none" style="margin-right: 12px; max-width: 26px;">
		...
		{/each}
		<button class="btn btn-accent btn-lg my-3 m-auto" on:click={() => handleSubmit(conflict.id)}
			>Choisir ce panier</button
		>
	{/each}
</div>
