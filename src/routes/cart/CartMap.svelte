<script>
	...
	const determineLocation = (querystring) => {
		...
		if (
			map &&
			newPosition &&
			(newPosition.latitude !== position.latitude || newPosition.longitude !== position.longitude)
		) {
	...
	onMount(() => {
		...
		L.tileLayer("https://api.maptiler.com/maps/positron/{z}/{x}/{y}.png?key=xdycAkqlmra0OjZw2dPy", {
			attribution:
	...
	$: if (map) {
		...
		const coordonnates = $cart.selectedDeliveries.map((producer) =>
			L.marker([producer.delivery.address.latitude, producer.delivery.address.longitude], {
				...
				`<p style="margin: 0">${producer.producerName}</p><p style="margin: 0">${producer.delivery.address.zipcode} ${producer.delivery.address.city}</p>`
			)
		);
