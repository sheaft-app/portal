...
<div class="pb-2">
	...
	<form>
		...
		{:else}
			<Slider
				start={selected ? data.deliveries.findIndex((d) => d.kind == selected.kind) : 0}
				{slides}
				let:slide={delivery}
				let:index
			>
				<div class="w-full">
					<div>
						<div class="uppercase font-normal mb-5 lg:mr-5 text-center">
							...
							{#if delivery.distanceInfos}
								<div
									class="distance-badge-content text-sm flex justify-center mt-2"
									style="color: {delivery.distanceInfos.color};"
								>
					...
					<div>
						{#each delivery.deliveryHours as deliveryHour, index}
							<div
								class:active={(!delivery.address ||
									(delivery.address &&
										selected &&
										selected.address &&
										delivery.address.line1 === selected.address.line1)) &&
									selectedDeliveryHour &&
									deliveryHour.from === selectedDeliveryHour.from &&
									deliveryHour.to === selectedDeliveryHour.to &&
									deliveryHour.expectedDeliveryDate === selectedDeliveryHour.expectedDeliveryDate}
