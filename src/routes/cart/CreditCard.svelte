<script>
    ...
    */
	...
	onMount(function () {
		...
		if (cardNumberEl) cardNumberEl.focus();
	});

	const handleSubmit = () => {
		paymentForm.validate();

		if ($paymentForm.valid) dispatch("submit");
	};
	...
	$: {
		...
		if ($card.data.cardNumber) {
			for (let index = 0; index < $card.data.cardNumber.length; index++) {
				if (cardNumberMask[index] == " " && $card.data.cardNumber[index] !== " ")
					$card.data.cardNumber =
						$card.data.cardNumber.substr(0, index) +
						" " +
						$card.data.cardNumber.substr(index, $card.data.cardNumber.length - index);
			}
			if ($card.data.cardNumber.substr("-1") == " ")
				$card.data.cardNumber = $card.data.cardNumber.substr(0, $card.data.cardNumber.length - 1);
			...
			{#if showCard}
				<div class="card-input">
					...
					<input
						...
						use:bindClass={{ form: paymentForm, name: "number" }}
				...
				<div class="card-input">
					...
					<input
						...
						use:bindClass={{ form: paymentForm, name: "name" }}
						data-ref="cardName"
						autocomplete="off"
					/>
					...
					<div class="card-form__col">
						<div class="card-form__group">
							...
							<select
								...
								use:bindClass={{ form: paymentForm, name: "month" }}
								data-ref="cardDate"
							>
							...
							<select
								...
								use:bindClass={{ form: paymentForm, name: "year" }}
								data-ref="cardDate"
							>
					...
					<div class="card-form__col cvv">
						<div class="card-input">
							...
							<input
								...
								v-mask="'####'"
								maxlength="4"
								use:bindClass={{ form: paymentForm, name: "cvx" }}
				...
				{/if}
				<button
					class="btn btn-lg btn-primary w-full shadow-xl justify-center text-xl mt-3"
					disabled={isPaying || !$paymentForm.valid}
					class:disabled={isPaying || !$paymentForm.valid}
