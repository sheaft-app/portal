<script>
	...
	onMount(async () => {
		...
		var firstTimeOnCartCookie = JSON.parse(localStorage.getItem("user_first_time_on_cart"));
	...
	const getProducerDeliveries = async () => {
		...
		await query({
			...
			success: (res) => {
				...
				const deliveriesIds = res.data
					.map((r) => r.deliveries)
					.flat()
					.map((d) => d.id);
				const cartItemWithProducerDeliveryNotFound = $cart.selectedDeliveries.find(
					(d) => d.delivery && d.delivery.id && !deliveriesIds.includes(d.delivery.id)
				);
	...
	$: isValid =
		$cart.products.find((c) => !c.producer.disabled) &&
		$cart.selectedDeliveries.length == cart.getProducersIds().length;
	...
	const validateCart = () => {
		if (!isValid) return blink($cart.products.filter((c) => !c.producer.deliveryHour));
		...
		<form on:submit|preventDefault={() => {}}>
			<div class="pb-6">
				...
				{#if productsHaveBeenRemoved}
					<div
						class="py-5 px-3 md:px-6 overflow-x-auto bg-orange-200 shadow
            ...
            rounded w-full mb-2 mt-2"
				...
				{#if $cart.products.length > 0}
					{#if firstTimeOnCart}
						<div
							class="py-5 px-3 md:px-6 overflow-x-auto bg-green-100 shadow
            rounded w-full mb-2 mt-2"
						>
							<p class="uppercase font-bold leading-none">Ok, et maintenant ?</p>
							<ul class="mt-4">
							...
							{#each cart.getSortedProductsByProducerName($cart.products) as product, i (product.id)}
								{#if i === 0 || cart.getSortedProductsByProducerName()[i - 1].producer.name !== product.producer.name}
									<p
										...
										class="font-semibold uppercase text-sm border border-gray-400 py-2 pl-3 bg-gray-100"
										class:mt-5={i >= 1}
										class:bg-orange-200={product.producer.disabled}
									>
										<span
                  ...
                  justify-center m-auto"
							...
							{#if hasSubmitError}
								<ErrorCard displayIcon={false} {errorsHandler} classes="mt-10">
									<p>Une erreur est survenue pendant l'envoi de la commande, veuillez réessayez.</p>
								</ErrorCard>
							{/if}
						</div>
					</div>
