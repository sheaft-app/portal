<script>
	...
	onMount(() => {
		if (typeof user.countryOfResidence == "object") {
			user.countryOfResidence = user.countryOfResidence.code;
		}
		if (typeof user.nationality == "object") {
...
{:else}
	<div in:fly|local={{ x: 300, duration: 300 }}>
		...
		<div class="bg-white shadow lg:rounded lg:mt-5 lg:mb-5">
			<div
				class="px-5 py-3 bg-gray-100 border-b border-gray-400 lg:rounded-t font-semibold flex justify-between items-center"
			>
				<p>Panier</p>
