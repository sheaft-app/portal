<script>
	...
	import { notificationsCount, displayNotificationCenter } from "./../notifications/store.js";
...
>
      ...
      rounded-b-lg right-0 user-panel"
		>
			...
			{#if $authRegistered && isInRole($authUserAccount, [Roles.Producer.Value])}
				...
				>
					<Icon data={ExternalRoutes.ProducerDetails.Icon} class="inline mr-4 md:mr-1 w-4" scale="0.9" />
			...
			{#if $authRegistered && isInRole($authUserAccount, [Roles.Store.Value])}
				...
				>
					<Icon data={ExternalRoutes.StoreDetails.Icon} class="inline mr-4 md:mr-1 w-4" scale="0.9" />
			...
			>
				<Icon data={AccountRoutes.Profile.Icon} class="inline mr-4 md:mr-1 w-4" scale="0.9" />
	...
	{/if}
	<div class="hidden lg:flex items-center justify-center -ml-8" style="min-width: 200px;">
		<button class="btn-link" on:click={() => goToHome(authInstance, routerInstance)}>
			<SheaftSvg width="150" />
		</button>
	</div>
	...
	<div class="justify-end inline-flex lg:items-center">
		{#if $cart.products.length > 0 && isInRole($authUserAccount, [Roles.Consumer.Value])}
			<button
				on:click={() => {
					userMenuExpended.set(false);
					displayNotificationCenter.set(false);
		...
		<div class="w-full">
			<ul class="nav__menu">
				{#if isInRole($authUserAccount, [Roles.Store.Value])}
					...
					<NavLink route={QuickOrderRoutes.Quick} />
					<hr class="my-2 mx-4" />
