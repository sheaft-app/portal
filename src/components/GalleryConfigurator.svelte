<script>
	...
	onMount(() => {
		...
		elements = elements.map((p) => {
			p.position = position;
			if (!p.id) p.id = incrementer.next().value;
		...
		let sortable = Sortable.create(el, {
			...
			onEnd: (evt) => {
				...
				elements = elements.map((p) => {
					if (p.position >= evt.newIndex && p.id !== id) p.position++;
					else if (p.id === id) p.position = evt.newIndex;
	...
	<div id="elements" class="flex flex-wrap">
		{#if elements && elements.length > 0}
			{#each elements as element}
				<div
					data-id={element.id}
					class="element preview relative m-2 shadow"
					style="background: url('{element.data
						? element.data
						: element[defaultSrcProperty]}'); height: {elementHeight}px; width:{elementWidth}px;"
				>
					<div
						class="absolute cursor-pointer btn-outline rounded-full remove-element"
		...
		{/if}
		<div
			class="add-element element cursor-pointer p-6 m-2 rounded-md btn-outline hover:btn-accent"
			on:click={() => addElement()}
