...
<div class={`w-6 h-6 rounded-full text-white inline-flex items-center justify-center bg-${data.Color}`}>
    <Icon
		data={data.Icon}
		scale=".7"
		spin={status === PurchaseOrderStatusKind.Processing.Value || status === PurchaseOrderStatusKind.Waiting.Value || status === AgreementStatusKind.WaitingForProducerApproval.Value || status === AgreementStatusKind.WaitingForStoreApproval.Value}
		/>
</div>
