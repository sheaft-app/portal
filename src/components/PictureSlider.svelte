<script>
	...
	export let elements = [],
		allowTouchMove = true,
		classes = "w-full shadow-md h-40 lg:h-64";
...
>
	{#each elements as element}
		<SwiperSlide>
			...
			{/if}
			<div
				style="background-image: url({element.url});background-size: contain;background-repeat:no-repeat;background-position: center;height:100%;{element.url.includes(
					'pictures/tags/'
				)
					? 'filter:blur(2px)'
					: ''}"
...
<div class="swiper-button-next bg-accent swiper-slide" on:click={next} class:hidden={isEnd} />
