<script>
    ...
    export let closings = [], mustBeInterval = false;
        ...
        <div transition:slide|local>
			...
			<div class="form-control justify-between" class:justify-end={!mustBeInterval}>
                {#if !mustBeInterval}
                    <Toggle
                        labelPosition="left"
                        classNames="ml-1"
                        bind:isChecked={closing.isInterval}>
                        Intervalle
                    </Toggle>
                {/if}
                <button type="button" 
            ...
            <div class="my-2">
                ...
                {#if closing.isInterval}
                    ...
                    <DatePickerWrapper bind:dateChosen={closing.dirtyTo} bind:selected={closing.to} props={{ start: closing.from }} />
                    </div>
                {/if}
                <div class="w-full form-control ml-2 mt-2">
                    <input
                        bind:value={closing.reason}
                        type="text"
                        placeholder="Donnez une raison pour la fermeture" />
                </div>
            </div>
        </div>
    {/each}
